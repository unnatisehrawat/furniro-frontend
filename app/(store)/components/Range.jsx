"use client"

import axios from "axios";
import { useEffect, useState } from "react"
import Image from "next/image";
import Link from "next/link"; 

export default function Range(){
    const [categories , setCategories] = useState([]);

    useEffect(() => {
        getCategories()
    }, [])

    async function getCategories(){
        try {
            const {data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/categories`)
            setCategories(data)
        } catch (error) {
            console.log(error)
        }
    }

    const displayedCategories = categories.slice(0, 3);

    return(
        <>
        <section className="max-w-7xl mx-auto py-20 bg-admin-bg">
            <h2 className="text-4xl font-bold text-center">Browse the Range</h2>
            <p className="text-gray-500 text-center mt-3">
                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
            </p>
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8 mt-8 sm:mt-14 px-4 sm:px-0">
                {displayedCategories.map((category) =>(
                    
                    <Link href={`/shop?category=${category._id}`} key={category._id}>
                        <div className="flex flex-col items-center cursor-pointer">
                            <div className="relative w-full h-[180px] sm:h-[400px]">
                                <Image 
                                    src={category.image}
                                    alt={category.name}
                                    fill
                                    className="object-cover hover:scale-105 transition duration-300 rounded-xl sm:rounded-2xl"
                                />
                            </div>
                            <h3 className="text-base sm:text-2xl font-semibold mt-3 sm:mt-6 text-center">{category.name}</h3>
                        </div>
                    </Link>

                ))}
            </div>

            <div className="flex justify-center mt-14">
                <Link
                    href="/categories"
                    className="border border-brand text-brand px-16 py-3 font-semibold hover:bg-brand hover:text-white transition cursor-pointer"
                >
                    Show More
                </Link>
            </div>
        </section>
        </>
    )
}
